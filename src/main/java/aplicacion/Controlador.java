...
public class Controlador {
    ...
    private ObjectId referenciaMongo;
    ...
    public boolean crearUsuario(Document infoUsuario){
        ...
        try{
            ...
            infoUsuario.remove("contrasena");
            this.usuario = infoUsuario;

            this.carrito = new Document("_id", new ObjectId())
                    .append("referenciaUsuario", infoUsuario.getObjectId("_id"))
    ...
    private void guardarUsuarioRedis(Document infoUsuario){
        try{
            if (infoUsuario == null) {
                logger.info("Error en la creación del usuario en Redis: infoUsuario es null.");
                return;
            }

            Map<String, String> hash = new HashMap<>();
            hash.put("contrasena", infoUsuario.getString("contrasena"));
            hash.put("mongoRef", infoUsuario.getObjectId("_id").toHexString());// redis no permite guardar BSON ObjectID
        ...
        try{
            ...
            if (!valores_campo.isEmpty()){
                if (contrasena.equals(valores_campo.get("contrasena"))){
                    ...
                    this.referenciaMongo = new ObjectId(valores_campo.get("mongoRef"));
                    return true;
                }
                else {
    ...
    private Document buscarUsuarioMongo(ObjectId refMongo){
    ...
    public void agregarProductoCarrito(ObjectId idProducto, int cantidad){
    ...
    public Document buscarProductoMongo(ObjectId idProducto){
        try{
            MongoService mongoService = new MongoService("productos");
    ...
    public Object[][] datosTablaUsuarios(){
        try{
            ...
            for (int i = 0; i < usuarios.size(); i++) {
                ...
                data[i][0] = (usuario.getObjectId("_id")).toHexString();
